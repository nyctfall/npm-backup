  ...
  const searchUserPkg = async (pkg: string): Promise<UserSearchNPMPkgResponce> => {
    ...
    try {
      ...
      try {
        ...
        const [ tableHead, ...searchResults ]: string[] = searchResponce.split(NPMPkg.NPMSearchResultRegex)
    ...
    try {
      ...
      if (answer.isNo){
        ...
        if (pkgFound){
          ...
          const trySubstitutePkg: Answer = Controverter.interpret(await promiseQuestion("Would you like to re-enter package name? (y/N): "))
