 ...
 */
 ...
 export class ExitCodeError extends Error {
   ...
   constructor(messageOrExitCode?: string | number, exitCode?: number){ 
     ...
     if (typeof messageOrExitCode === "number") this.code = Number(messageOrExitCode) << 0 // bit-wise shift by 0 is used to make it a 32-bit integer, without changing the vaule... to much.
     else {
       if (messageOrExitCode) this.message = String(messageOrExitCode)
       if (exitCode) this.code = Number(exitCode) << 0 // bit-wise shift by 0 is used to make it a 32-bit integer, without changing the vaule... to much.
     }
 ...
 export class OperationError extends ExitCodeError {
   ...
   constructor(messageOrExitCode?: string | number, exitCodeAfterMessage?: number){ 
     ...
     if (typeof messageOrExitCode === "number") this.code = Number(messageOrExitCode) << 0 // bit-wise shift by 0 is used to make it a 32-bit integer, without changing the vaule... to much.
       ...
       if (messageOrExitCode) this.message = String(messageOrExitCode)
       if (exitCodeAfterMessage) this.code = Number(exitCodeAfterMessage) << 0 // bit-wise shift by 0 is used to make it a 32-bit integer, without changing the vaule... to much.
     }
 ...
 export class SuccesfulNonError extends Error {
