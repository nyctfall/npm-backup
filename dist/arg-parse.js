...
exports.options = exports.command = exports.argParser = exports.argParserQ = exports.commander = exports.Command = void 0;
const commander_1 = require("commander");
const arg_parse_utils_1 = require("./arg-parse-utils");
const op_queue_pipeline_1 = require("./op-queue-pipeline");
var commander_2 = require("commander");
Object.defineProperty(exports, "Command", { enumerable: true, get: function () { return commander_2.Command; } });
/**
 * @fileoverview - Read the CLI input to get commands, arguments, and options.
 */
/** */
const verboseHandler = (placeholder, previousVerbosityLevel) => {
    void placeholder; // used to satisfy typescript
    return Number(previousVerbosityLevel) + 1;
};
const dbgHandler = (placeholder, previousDebugLevel) => {
    void placeholder; // used to satisfy typescript
 ...
 *
 */
exports.commander = (0, commander_1.createCommand)("npm-super-pack")
    .version("0.1.0")
    ...
    .addOption(new commander_1.Option("-P, --save-prod, --production=true, --production", "Save the production dependencies of the package(s)."))
    .addOption(new commander_1.Option("-B, --save-bundle", "Save the bundled dependencies of the package(s), this is the normal behavior of npm pack."))
    .addOption(new commander_1.Option("-D, --save-dev, --production=false", "Save both the production and dev dependencies of the package(s).")
    .implies({ saveProd: true }))
    .addOption(new commander_1.Option("-O, --save-optional", "Save the production, dev, and optional dependencies of the package(s).")
    .implies({ saveProd: true, saveDev: true }))
    .addOption(new commander_1.Option("--save-peer", "Save the peer dependencies of the package(s).")
    .implies({ saveProd: true }))
    .addOption(new commander_1.Option("--save <path>", "Install package to file system location, also saves package to NPM's internal cache (_cacache).").conflicts("noSave"))
    .addOption(new commander_1.Option("--dest <path>", "Install package to file system location, also saves package to NPM's internal cache (_cacache).").conflicts("noSave"))
    .addOption(new commander_1.Option("--pack-destination <path>", "Install package to file system location, also saves package to NPM's internal cache (_cacache).").conflicts("noSave"))
    .addOption(new commander_1.Option("--package-destination <path>", "Install package to file system location, also saves package to NPM's internal cache (_cacache).").conflicts("noSave"))
    .addOption(new commander_1.Option("--no-save", "Does not install package, only saves package to NPM's internal cache (\"_cacache\").").conflicts("save"))
    ...
    .addOption(new commander_1.Option("--debug", "Print debug information, use repeatedly to print more information.")
    ...
    .addOption(new commander_1.Option("--omit <dependency types...>", '"prod", "dev", "optional", "bundle", or "peer" (can be set multiple times). Dependency types to omit from the installation. If a package type appears in both the --include and --omit lists, then it will be included.')
    .choices(["prod", "production", "dev", "optional", "bundle", "bundled", "peer"]))
    .addOption(new commander_1.Option("--include <dependency types...>", '"prod", "dev", "optional", "bundle", or "peer" (can be set multiple times). Allows for defining which types of dependencies to install. This is the inverse of --omit=<type>. Dependency types specified in --include will not be omitted, regardless of the order in which omit/include are specified on the command-line.')
    .choices(["prod", "dev", "optional", "peer"]))
    .addOption(new commander_1.Option("--strict-peer-deps", "If set to true, and --legacy-peer-deps is not set, then any conflicting peerDependencies will be treated as an install failure.")
    .conflicts("legacyPeerDeps"))
    .addOption(new commander_1.Option("--legacy-peer-deps", "Causes npm to completely ignore peerDependencies, as in npm versions 3 through 6. If a package cannot be installed because of overly strict peerDependencies that collide, it provides a way to move forward resolving the situation. Use of legacy-peer-deps is not recommended, as it will not enforce the peerDependencies contract that meta-dependencies may rely on.")
    ...
    .addOption(new commander_1.Option("--loglevel <level>, --log-level <level>, --npm-log-level <level>", "The logging detail NPM uses.")
    .choices(["silent", "error", "warn", "notice", "http", "timing", "info", "verbose", "silly"]));
/**
 *
 */
exports.argParserQ = new op_queue_pipeline_1.OpsPipeline("Argument Parser" /* , { useDebug: true, useVerbose: false } */)
    .pipe(({ argv, parser }) => {
    ...
    .pipe(({ argv, parser }) => {
    return {
        args: (0, arg_parse_utils_1.argvSanitize)(argv),
        parser
    };
}, "Input Sanitization")
    .pipe(({ args, parser }) => {
 ...
 *
 */
const argParser = (argv = process.argv, parser = exports.commander, Q = exports.argParserQ) => {
 ...
 *
 */
const command = async (argv = process.argv.slice(2)) => {
    return (await (0, exports.argParser)(argv, exports.commander)).pipe[0];
};
exports.command = command;
/**
 *
 */
exports.options = (exports.command)();
/*
