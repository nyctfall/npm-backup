...
exports.OpsPipeline = exports.opCurry = exports.op = void 0;
__exportStar(require("./pipeline-types"), exports);
/**
    ...
    try {
        ...
        if (useShell && typeof e === "object" && e !== null) {
            if ("status" in e && typeof e.status === "number") {
                output.exitCode = e.status; // as statement used to satisfy TS.
            }
            else if ("code" in e && typeof e.code === "number") {
                output.exitCode = e.code; // as statement used to satisfy TS.
            }
        }
...
const opCurry = (fn, env) => {
    return (input) => {
        return (0, exports.op)(fn, input, env);
    };
};
exports.opCurry = opCurry;
/**
...
class OpsPipeline {
    constructor(description, env) {
        ...
        if (this.env.useDebug) {
            ...
            if (this.env.useNestingDebug)
                this.trace.nestedTraces = [];
        }
    }
    /**
    ...
    */
    formatOp(operation, env) {
        ...
        // otherwise, treat it as an Op:
            ...
            return (0, exports.opCurry)(operation, env);
        }
        // do nothing if it can't be used in the pipeline...
    }
    pipe(operation, description, envExtras) {
        if (!this.isMutable)
            throw Error(`Attempted to add fallback Operation to a locked Pipeline.`); // return this
    ...
    fallback(fallback, description, envExtras) {
        if (!this.isMutable)
            throw Error(`Attempted to add fallback Operation to a locked Pipeline.`); // return this
        if (this.queue.length < 1)
            return this; //  throw Error(`Attempted to add fallback Operation without a setting a preceeding Operation in the Pipeline.`)
        // is a function, and isn't a pipeline:
        ...
        if (isPipeline || isOp) {
            ...
            if (enqueueable) {
                ...
                const flOpsList = this.fallbackOps.get(targetForFailsafe);
        ...
        try {
            ...
            for (const nextOp of this.queue) {
                ...
                if (this.env.useDebug && this.trace instanceof Object) {
                    ...
                    if (this.env.useNestingDebug && "debugBackTrace" in output) {
                        // the  as Type  expression is used to satisfy TypeScript:
                        this.trace.nestedTraces?.push(output.debugBackTrace);
                    }
                }
                // exited successfully:
                ...
                // use FlOps, it there's an entry:
                    ...
                    let flIndex = 0;
                    // fallback Ops:
                    const flOpsQueue = this.fallbackOps.get(nextOp);
